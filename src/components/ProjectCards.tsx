...
// ── Dev card face — fixed proportions: 240×310, image 60% ─────────────────────
function DevCardFace({ project }: { project: DevProject }) {
  return (
    <div className="rounded-2xl overflow-hidden bg-white dark:bg-[#141414] border border-gray-200 dark:border-white/[0.08] select-none" style={{ width: 240, boxShadow: "0 10px 40px rgba(0,0,0,0.15), 0 2px 8px rgba(0,0,0,0.08)" }}>
      {/* Image — fixed 150px tall */}
      <div className="relative overflow-hidden bg-gray-100 dark:bg-white/5" style={{ height: 148 }}>
      ...
      {/* Content — fixed height to make total ~310px */}
      <div className="p-3" style={{ height: 130 }}>
        <h3 className="font-black text-sm text-gray-800 dark:text-gray-100 leading-tight mb-0.5">{project.title}</h3>
        <p className="text-[10px] font-medium mb-2" style={{ color: project.accent }}>{project.subtitle}</p>
        ...
        <p className="text-[9px] text-gray-300 dark:text-white/20 italic mt-2">click to expand</p>
      </div>
    </div>
  );
}

// ── Design card face — portrait 160×280, image 65% ────────────────────────────
function DesignCardFace({ card }: { card: DesignCard }) {
  return (
    <div className="rounded-2xl overflow-hidden bg-white dark:bg-[#141414] border border-gray-200 dark:border-white/[0.08] select-none" style={{ width: 160, boxShadow: "0 10px 40px rgba(0,0,0,0.15)" }}>
      {/* Visual — portrait proportion, 180px */}
      <div className="flex items-center justify-center relative overflow-hidden" style={{ height: 180, background: `linear-gradient(155deg, ${card.accent}28, ${card.accent}08)` }}>
        ...
        <span className="text-4xl relative z-10" style={{ filter: `drop-shadow(0 0 10px ${card.accent}99)` }}>{card.icon}</span>
        <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: card.accent }} />
      </div>
      {/* Title + tags — 90px */}
      <div className="p-2.5" style={{ height: 90 }}>
        <h3 className="font-black text-xs text-gray-800 dark:text-gray-100 leading-tight mb-1.5">{card.title}</h3>
        <div className="flex flex-wrap gap-1">
        ...
        <p className="text-[9px] text-gray-300 dark:text-white/20 italic mt-1.5">click to expand</p>
      </div>
    </div>
  );
}
...
function scatter(idx: number, total: number, W: number, H: number, cW: number, cH: number) {
  ...
  // Evenly space cards across the full area
  const cellW = (W - cW) / Math.max(cols - 1, 1);
  const cellH = (H - cH) / Math.max(rows - 1, 1);
  // Deterministic jitter using prime multipliers
...
export default function ProjectsSection() {
  ...
  // Card + table sizes — proportions carefully matched to card content
  // Dev: 240w × 278h card  →  table tall enough for 2 rows with breathing room
  const DEV_W = 240, DEV_H = 278, DEV_TABLE_H = 580;
  // Design: 160w × 270h card  →  table tall enough for 2 rows
  const DES_W = 160, DES_H = 270, DES_TABLE_H = 520;
