...
// ── Breakpoint hook — identical to ExperienceSection ──────────────────────────
...
function useBreakpoint(): Breakpoint {
  ...
  useEffect(() => {
    ...
    calc(); // correct immediately on mount
...
// ── Modal — pattern mirrors ExperienceSection.Modal exactly ───────────────────
//
// The key fix: `sheet` is computed in the PARENT (ProjectsSection) from
// `useBreakpoint()` and passed as a stable boolean prop — exactly how
// ExperienceSection does it. This is the only correct pattern because:
//
//   • useBreakpoint() calls setBp() synchronously in its first useEffect,
//     so by the time the user taps a card and React renders Modal, bp is
//     already the real viewport value — not the SSR default "lg".
//
//   • Passing sheet as a prop means Modal's useEffect (animation) sees
//     the correct value immediately on its first run — no null guard,
//     no two-render delay, no flash of wrong position.
//
// Additional fixes kept from previous iteration:
//   • overflow-hidden class removed from panel — conflicts with overflowY:auto
//     on iOS Safari and kills scrolling inside the sheet.
//   • body.overflow managed only inside Modal cleanup (not in parent).
//   • close() uses a single gsap.timeline so onComplete fires exactly once.
//   • closing ref guards against double-invoke (Escape + backdrop tap).
...
function Modal({
  ...
  data: ModalData;
  onClose: () => void;
  sheet: boolean;   // ← passed from parent, always correct, never stale
}) {
  const overlayRef = useRef<HTMLDivElement>(null);
  const panelRef   = useRef<HTMLDivElement>(null);
  const closing    = useRef(false);

  // Freeze body scroll — managed only here, cleaned up here
  useEffect(() => {
  ...
  // Enter animation — sheet is a stable prop so this fires once with the
  // correct value, same as ExperienceSection
  useEffect(() => {
  ...
  const close = useCallback(() => {
    ...
    // Single timeline — onComplete fires once when the longer tween ends
    if (sheet) {
  ...
  return (
    ...
    >
      <div
        ref={panelRef}
        // ⚠️  No overflow-hidden class — breaks overflowY:auto on iOS Safari.
        //     Border-radius set via style (not rounded-*) for clean conditional.
...
export default function ProjectsSection() {
  ...
  // ── bp and sheet computed here in the parent — identical to ExperienceSection
  ...
  return (
    <section id="projects" ref={sectionRef} className="relative min-h-screen py-24 px-6 sm:px-10 lg:px-16 xl:px-24">
      {/* sheet passed as stable prop — same pattern as ExperienceSection */}
